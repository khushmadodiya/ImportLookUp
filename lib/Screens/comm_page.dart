...
class CommApealCases extends StatefulWidget {
  const CommApealCases({super.key});

  @override
  State<CommApealCases> createState() => _CommApealCasesState();
}

class _CommApealCasesState extends State<CommApealCases> {
  @override
  void initState() {
    super.initState();
  ...
  Widget build(BuildContext context) {
    ...
    for (int i = 0; i < asseserProvider.assesers()!.length; i++) {
      ...
      if (asseser['subcategory'] == 'Commissioner Appeal') {
        print(asseser['subcategory']);
        myData.add(asseser);
        num++;
        ...
        child: Column(
          children: [
             InkWell(
              ...
              onTap:(){
                ExcelDonwloadOption().exportToExcel(myData,'Commissioner Appeal');
              },
            ),
